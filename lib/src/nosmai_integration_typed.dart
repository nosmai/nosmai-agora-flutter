...
/// Typed Nosmai Integration with proper error handling and type safety
class NosmaiIntegrationTyped {
  static const platform = MethodChannel('nosmai_agora');

  // SDK Initialization
  static Future<bool> initNosmai(String licenseKey) async {
    try {
      final bool result = await platform.invokeMethod('initNosmai', {
  ...
  // Streaming Control
  static Future<bool> startCustomCamera(
    ...
    try {
      ...
      print('Error starting custom camera: $e');
      throw NosmaiError.camera(
        type: NosmaiErrorType.cameraUnavailable,
        message: 'Failed to start custom camera',
        details: e.toString(),
      );
    }
  }

  static Future<bool> stopCustomCamera() async {
    try {
      final bool result = await platform.invokeMethod('stopCustomCamera');
      return result;
    } catch (e) {
      print('Error stopping custom camera: $e');
      return false;
    }
  }
  ...
  static Future<bool> startPreview() async {
    try {
      final bool result = await platform.invokeMethod('startPreview');
      return result;
    } catch (e) {
  ...
  static Future<bool> stopPreview() async {
    try {
      final bool result = await platform.invokeMethod('stopPreview');
      return result;
    } catch (e) {
  ...
  // Flash and Torch Methods with typed enums
  ...
  static Future<bool> applyEffect(String path) async {
    try {
      final ok =
          await platform.invokeMethod<bool>('applyEffect', {'path': path});
      return ok == true;
    } catch (e) {
  ...
  static Future<bool> removeAllEffects() async {
    try {
      final ok = await platform.invokeMethod<bool>('removeAllEffects');
      return ok == true;
    } catch (_) {
      return false;
    }
  ...
  // Cloud filters methods with typed models
  ...
  static Future<bool> removeAllFilters() async {
    try {
      final bool result = await platform.invokeMethod('removeAllFilters');
      return result;
    } catch (e) {
      print('Error removing all filters: $e');
      return false;
    }
  }
  ...
  static Future<bool> applySharpen(double sharpen) async {
    try {
      final clamped = sharpen.clamp(0.0, 1.5);
      final bool result = await platform.invokeMethod('applySharpen', {
        'sharpen': clamped,
      });
      return result;
    } catch (e) {
      print('Error applying sharpen: $e');
      return false;
    }
  }
  ...
  // HSB Adjustment Methods with typed model
